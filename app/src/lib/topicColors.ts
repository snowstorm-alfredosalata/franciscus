/**
 * Single source of truth for topic-type pill colors (virtue / theme / event /
 * place / person). Previously copy-pasted across the topics and reader pages.
...
// theme moved blue→cyan and event amber→orange so they no longer read as the
...
const base: Record<string, string> = {
	...
	theme: 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900 dark:text-cyan-300',
...
const hover: Record<string, string> = {
	...
	theme: 'hover:bg-cyan-200 dark:hover:bg-cyan-800',
...
/** Color classes for an topic type. Pass `true` to include hover variants (for links). */
export function topicColors(type: string, withHover = false): string {
